...
import { NotebookConcatTextDocument } from "vscode";
...
export class InteractiveConcatTextDocument implements IConcatTextDocument {
  ...
  get isComposeDocumentsAllClosed(): boolean {
    return (
      !this._notebook.getCells().some((cell) => !cell.document.isClosed) &&
      !!this._input?.isClosed
    );
  }
  ...
  constructor(
    ...
    this._concatTextDocument = notebookApi.createConcatTextDocument(
      _notebook,
      this._selector
    );
    console.log("Creating notebook concat");
    ...
    const counter = /Interactive-(\d+)\.interactive/.exec(
      this._notebook.uri.path
    );
    if (counter) {
      this._input = workspace.textDocuments.find(
        (document) =>
          document.uri.path.indexOf(`InteractiveInput-${counter[1]}`) >= 0
      );
    }
  ...
  getText(range?: Range): string {
    ...
    const endDocument = workspace.textDocuments.find(
      (document) => document.uri.toString() === end.uri.toString()
    );
    ...
    const a = startDocument.getText(
      new Range(start.range.start, new Position(startDocument.lineCount, 0))
    );
  ...
  offsetAt(position: Position): number {
    ...
    if (line >= this._lineCounts[0]) {
      ...
      return (
        this._input?.offsetAt(new Position(lineOffset, position.character)) ?? 0
      );
    }
    // concat
  ...
  positionAt(locationOrOffset: Location | number): Position {
    if (typeof locationOrOffset === "number") {
      ...
      return new Position(
        this._lineCounts[0] + position.line,
        position.character
      );
    }
  ...
  lineAt(posOrNumber: Position | number): TextLine {
    const position =
      typeof posOrNumber === "number"
        ? new Position(posOrNumber, 0)
        : posOrNumber;
    ...
    const cell = this._notebook
      .getCells()
      .find((c) => c.document.uri.toString() === location.uri.toString());
