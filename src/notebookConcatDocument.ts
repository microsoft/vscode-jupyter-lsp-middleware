...
export class NotebookConcatDocument implements TextDocument, IDisposable {
  ...
  constructor(
    ...
    this.dummyFilePath = path.join(
      dir,
      `${NotebookConcatPrefix}${uuid().replace(/-/g, "")}.py`
    );
    this.dummyUri = Uri.file(this.dummyFilePath);

    if (notebook.uri.scheme === InteractiveScheme) {
      this.concatDocument = new InteractiveConcatTextDocument(
        notebook,
        selector,
        notebookApi
      );
    } else {
      this.concatDocument = new EnhancedNotebookConcatTextDocument(
        notebook,
        selector,
        notebookApi
      );
    ...
    this.onDidChangeSubscription = this.concatDocument.onDidChange(
      this.onDidChange,
      this
    );
  ...
  public getText(range?: Range | undefined): string {
    return range
      ? this.concatDocument.getText(range)
      : this.concatDocument.getText();
  }
  ...
  private onDidChange() {
    this._version += 1;
    const newUris = this.concatDocument
      .getComposeDocuments()
  ...
  private getPositionOfCell(cellUri: Uri): Position {
    return this.concatDocument.positionAt(
      new Location(cellUri, new Position(0, 0))
    );
  }
  ...
  private raiseCellDeletions(newUris: string[], oldUris: string[]) {
    ...
    const changes = oldIndexes.map((index) => {
      ...
      const endPosition = new Position(
        position.line + this.cellTracking[index].lineCount,
        0
      );
