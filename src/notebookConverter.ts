...
export class NotebookConverter implements Disposable {
  ...
  private activeDocuments: Map<string, NotebookConcatDocument> = new Map<
    string,
    NotebookConcatDocument
  >();

  private pendingCloseDocuments: Map<string, NotebookConcatDocument> = new Map<
    string,
    NotebookConcatDocument
  >();

  private activeDocumentsOutgoingMap: Map<string, NotebookConcatDocument> =
    new Map<string, NotebookConcatDocument>();
  ...
  public toIncomingDiagnosticsMap(
    uri: Uri,
    ...
    if (wrapper) {
      ...
      wrapper.concatDocument
        .getComposeDocuments()
        .forEach((document: TextDocument) => {
  ...
  public toIncomingWorkspaceSymbols(
  ...
  public toIncomingWorkspaceEdit(
  ...
  public toOutgoingPosition(cell: TextDocument, position: Position): Position {
    const concat = this.getConcatDocument(cell);
    return concat
      ? concat.positionAt(new Location(cell.uri, position))
      : position;
  }
  ...
  public toOutgoingRange(cell: TextDocument, cellRange: Range): Range {
    const concat = this.getConcatDocument(cell);
    if (concat) {
      const startPos = concat.positionAt(
        new Location(cell.uri, cellRange.start)
      );
  ...
  public toOutgoingOffset(cell: TextDocument, offset: number): number {
    ...
    if (concat) {
      ...
      const overallPosition = concat.positionAt(
        new Location(cell.uri, position)
      );
  ...
  public toOutgoingContext(
    cell: TextDocument,
    context: CodeActionContext
  ): CodeActionContext {
    return {
      ...context,
      diagnostics: context.diagnostics.map(
        this.toOutgoingDiagnostic.bind(this, cell)
      ),
    };
  }

  public toIncomingHover(
    cell: TextDocument,
    ...
    if (completions) {
      ...
      return {
        ...
        items: completions.items.map(
          this.toIncomingCompletion.bind(this, cell)
        ),
      };
    }
    return completions;
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  public toIncomingSymbols(
    cell: TextDocument,
    symbols: SymbolInformation[] | DocumentSymbol[] | null | undefined
  ) {
    if (symbols && Array.isArray(symbols) && symbols.length) {
      if (symbols[0] instanceof DocumentSymbol) {
        return (<DocumentSymbol[]>symbols).map(
          this.toIncomingSymbolFromDocumentSymbol.bind(this, cell)
        );
      }
      return (<SymbolInformation[]>symbols).map(
        this.toIncomingSymbolFromSymbolInformation.bind(this, cell)
      );
    }
    return symbols ?? undefined;
  }

  public toIncomingSymbolFromSymbolInformation(
    cell: TextDocument,
    ...
    return {
      ...
      relatedInformation: diagnostic.relatedInformation
        ? diagnostic.relatedInformation.map(
            this.toIncomingRelatedInformation.bind(this, cell)
          )
        : undefined,
    };
  }
  ...
  public toIncomingActions(
    _cell: TextDocument,
    actions: (Command | CodeAction)[] | null | undefined
  ): undefined {
  ...
  public toIncomingRename(
    ...
    if (rangeOrRename) {
      ...
      return {
        ...rangeOrRename,
        range: this.toIncomingLocationFromRange(cell, rangeOrRename.range)
          .range,
      };
    }
  ...
  public toIncomingPosition(
    cell: TextDocument | Uri,
    position: Position
  ): Position {
    // This is dangerous as the URI is not remapped (location uri may be different)
    return this.toIncomingLocationFromRange(cell, new Range(position, position))
      .range.start;
  }

  private getTextDocumentAtLocation(
    location: Location
  ...
  private toIncomingWorkspaceSymbol(
    symbol: SymbolInformation
  ): SymbolInformation {
  ...
  private toOutgoingDiagnostic(
    ...
    return {
      ...
      relatedInformation: diagnostic.relatedInformation
        ? diagnostic.relatedInformation.map(
            this.toOutgoingRelatedInformation.bind(this, cell)
          )
        : undefined,
    };
  }
  ...
  private toIncomingSymbolFromDocumentSymbol(
    cell: TextDocument,
  ...
  private locationNeedsConversion(locationUri: Uri): boolean {
    return (
      locationUri.scheme === NotebookCellScheme ||
      this.getWrapperFromOutgoingUri(locationUri) !== undefined
    );
  }
  ...
  private toIncomingLocationFromRange(
    cell: TextDocument | Uri,
    range: Range
  ): Location {
    const uri = cell instanceof Uri ? <Uri>cell : cell.uri;
  ...
  private toOutgoingContentChangeEvent(
    cell: TextDocument,
    ev: TextDocumentContentChangeEvent
  ) {
    return {
  ...
  private onDidCloseNotebook(doc: NotebookDocument) {
    if (this.notebookFilter.test(doc.uri.fsPath)) {
      ...
      this.activeDocumentsOutgoingMap.delete(
        NotebookConverter.getDocumentKey(wrapper.uri)
      );
  ...
  private getWrapperFromOutgoingUri(
    outgoingUri: Uri
  ): NotebookConcatDocument | undefined {
    return this.activeDocumentsOutgoingMap.get(
      NotebookConverter.getDocumentKey(outgoingUri)
    );
  }

  private getTextDocumentWrapper(
    cell: TextDocument | Uri
  ): NotebookConcatDocument {
    const uri = cell instanceof Uri ? <Uri>cell : cell.uri;
    ...
    if (!result) {
      const doc = this.api.notebookDocuments.find((n) =>
        this.fs.arePathsSame(uri.fsPath, n.uri.fsPath)
      );
      if (!doc) {
      ...
      result.onCellsChanged(
        (e) => this.onDidChangeCellsEmitter.fire(e),
        undefined,
        this.disposables
      );
      this.activeDocuments.set(key, result);
      this.activeDocumentsOutgoingMap.set(
        NotebookConverter.getDocumentKey(result.uri),
        result
      );
    }
    return result;
  ...
  private getConcatDocument(
    cell: TextDocument | Uri
  ): IConcatTextDocument | undefined {
    return this.getTextDocumentWrapper(cell)?.concatDocument;
  }

  private filterMagics(
    wrapper: NotebookConcatDocument,
    value: Diagnostic
  ): boolean {
